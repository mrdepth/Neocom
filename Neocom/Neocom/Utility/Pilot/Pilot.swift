...
extension Pilot {
    init(attributes: ESI.CharacterAttributes, skills: ESI.CharacterSkills, skillQueue: [ESI.SkillQueueItem], implants: [Int], context: NSManagedObjectContext) {
        ...
        for implant in implants {
            guard let type = try? context.from(SDEInvType.self).filter(\SDEInvType.typeID == implant).first() else {continue}
        ...
        for skill in validSkillQueue[..<i] {
            guard let endSP = try? skill.levelEndSP ?? context.from(SDEInvType.self).filter(\SDEInvType.typeID == skill.skillID).first().flatMap({Pilot.Skill(type: $0)})?.skillPoints(at: skill.finishedLevel) else {continue}
        ...
        let sq = validSkillQueue[i...].sorted{$0.queuePosition < $1.queuePosition}.compactMap { i -> Pilot.SkillQueueItem? in
            guard let type = try? context.from(SDEInvType.self).filter(\SDEInvType.typeID == i.skillID).first(), let skill = Pilot.Skill(type: type) else {return nil}
