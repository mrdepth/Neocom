...
- (void)viewDidLoad
{
	...
	[self.mailBox loadBodyForMessage:self.message withCompletionBlock:^(EVEMailBodiesItem *body, NSError *error) {
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			[template replaceOccurrencesOfString:@"{text}"
									  withString:body.text ?: error ? [error localizedDescription] : NSLocalizedString(@"Can't load the message body.", nil)
										 options:0
										   range:NSMakeRange(0, template.length)];
			...
			if (body) {
				[self.mailBox markAsRead:@[self.message]];
			};
		});
	}];
}
