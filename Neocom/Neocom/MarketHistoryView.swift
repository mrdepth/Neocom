...
class MarketHistoryView: UIView {
	...
	var donchianVisibleRange: CGRect? {
		didSet {
			if let donchianVisibleRange = donchianVisibleRange {
				...
				let h = bounds.size.height / (1.0 - MarketHistoryView.ratio)
	...
	var volume: UIBezierPath? {
		didSet {
			if let volume = volume {
				...
				let h = bounds.size.height / MarketHistoryView.ratio
	...
	func drawVolume(canvas: CGRect) {
		...
		if rect.size.width > 0 && rect.size.height > 0 {
			...
			transform = transform.scaledBy(x: canvas.size.width / rect.size.width, y: canvas.size.height / rect.size.height * MarketHistoryView.ratio)
	...
	func drawDonchianAndMedian(canvas: CGRect) {
		...
		if rect.size.width > 0 && rect.size.height > 0 {
			...
			transform = transform.scaledBy(x: 1, y: -1)
			transform = transform.translatedBy(x: 0, y: -canvas.size.height * (1.0 - MarketHistoryView.ratio))
			transform = transform.scaledBy(x: canvas.size.width / rect.size.width, y: canvas.size.height / rect.size.height * (1.0 - MarketHistoryView.ratio))
			transform = transform.translatedBy(x: -rect.origin.x, y: -rect.origin.y)
	...
	func drawGrid(canvas: CGRect) {
		...
		let gridSize = MarketHistoryView.gridSize
		var y = 0 as CGFloat
		let grid = UIBezierPath()
		
		let attributes: [NSAttributedString.Key: Any] = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 10), NSAttributedString.Key.foregroundColor: UIColor.white]
		...
		while y <= canvas.size.height {
			...
			if (donchian > 0) {
				let s = NSAttributedString(string: UnitFormatter.localizedString(from: donchian, unit: .none, style: .short), attributes: attributes)
			...
			if y >= canvas.height * (1.0 - MarketHistoryView.ratio) && volume > 0 {
				let s = NSAttributedString(string: UnitFormatter.localizedString(from: round(volume), unit: .none, style: .short), attributes: attributes)
				var rect = s.boundingRect(with: size, options: [], context: nil)
		...
		while x <= canvas.size.width {
			...
			if m != month {
				...
				let s = NSAttributedString(string: MarketHistoryView.months[month - 1], attributes: attributes)
				var rect = s.boundingRect(with: size, options: [], context: nil)
