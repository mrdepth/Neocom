...
class NCKillmailVictimRow: NCContactRow {
	...
	init(character: NSManagedObjectID?, corporation: NSManagedObjectID?, alliance: NSManagedObjectID?, dataManager: NCDataManager) {
	...
	override func content() -> Future<TreeNode?> {
		...
		return DispatchQueue.global(qos: .utility).async { () -> TreeNode? in
			...
			return try NCDatabase.sharedDatabase!.performTaskAndWait { managedObjectContext in
				...
				ids.formUnion([victim.characterID, victim.corporationID, victim.allianceID].compactMap{$0}.map{Int64($0)})
				ids.formUnion(killmail.getAttackers().map { [$0.characterID, $0.corporationID, $0.allianceID].compactMap{$0}.map{Int64($0)}}.joined())
				...
				progress.perform {
					if !ids.isEmpty {
						contacts = try? dataManager.contacts(ids: ids).get()
					}
				}
				...
				progress.perform {
					if !typeIDs.isEmpty {
						do {
							let prices = try dataManager.prices(typeIDs: Set(typeIDs.keys)).get()
							typeIDs.forEach {
								cost += Double((prices[$0.key] ?? 0)) * Double($0.value)
							}
						}
						catch {
