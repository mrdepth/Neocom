...
struct TypeCell_Previews: PreviewProvider {
    static var previews: some View {
        List {
            ...
            TypeCell(type: (try! AppDelegate.sharedDelegate.persistentContainer.viewContext.from(SDEInvType.self).filter(Expressions.keyPath(\SDEInvType.dgmppItem?.requirements?.powerGrid) > 10000).first())!)
            TypeCell(type: (try! AppDelegate.sharedDelegate.persistentContainer.viewContext.from(SDEInvType.self).filter(Expressions.keyPath(\SDEInvType.dgmppItem?.damage) != nil).first())!)
